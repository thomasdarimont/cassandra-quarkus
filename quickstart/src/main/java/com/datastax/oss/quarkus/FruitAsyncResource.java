...
public class FruitAsyncResource {
  ...
  public Multi<Fruit> getAll() {
    return Multi.createFrom()
        .converter(MultiReactorConverters.fromFlux(), Flux.from(service.get(STORE_NAME)));
  }

  @POST
  public CompletionStage<Multi<Fruit>> add(FruitDto fruitDto) {
    Fruit fruit = covertFromDto(fruitDto);
    return service.add(fruit).thenApply(ignored -> getAll());
  }
