...
`quarkus.cassandra.metrics.session.enabled` and `quarkus.cassandra.metrics.node.enabled`
...
link:https://docs.datastax.com/en/developer/java-driver/latest/manual/core/configuration/reference/[driver settings reference]
page; search for the `advanced.metrics` section. Also, Cassandra driver metrics are explained in
detail in the
